...
Neele~Drobnitzky\authorOrcid{0000-0002-3181-941X}\authorRef{8}
...
\autlabel{8} Heidelberg University Biochemistry Center (BZH), Heidelberg, Germany,\\
}
...
CRC~1638 investigates cellular membrane remodelling across five institutions, with projects generating data spanning imaging, structural biology, lipidomics, proteomics, and simulations. As in much of experimental biology, RDM awareness among researchers varies widely and formal training in RDM was rarely part of academic curricula when most current researchers were trained. The INF project addresses this --- sensitising researchers to the importance of good RDM and helping them get started in a way that leads to meaningful and lasting change in daily practice.
A network of data stewards embedded within CRC groups forms the backbone of this approach, supported by INF seminars and one-to-one meetings with groups as needed. The seminars tackle topics that often feel overwhelming --- data organisation, persistent identifiers, data management plans --- and break them down into steps that stewards can act on easily within their groups. The aim is to show researchers why better RDM is worth doing, not to create bureaucratic overhead.
The main technical focus has been the electronic lab notebook (ELN) \href{https://www.elabftw.net/}{eLabFTW}. Support goes beyond introducing the tool: a starter pack of templates covering common lab resources such as plasmids, antibodies, and cell lines provides groups with a ready-made structure including metadata fields with controlled vocabularies and linking logic between entries, making good documentation the path of least resistance rather than an added burden. A practical barrier for groups with existing reagent databases was migration: the INF team developed \href{https://github.com/sfb1638/elAPI\_Plugins}{\textit{elAPI Plugins}}, which uses \href{https://github.com/uhd-urz/elAPI}{\textit{elAPI}} from the Heidelberg University Computing Center (URZ) to batch-generate ELN entries from pre-defined templates, making the migration a one-time task. Within one year, active eLabFTW use in the CRC grew significantly in terms of number of groups and extent of use with immediate perceived benefit to the users. The project collaborates with the Heidelberg URZ and Research Data Unit (RDU) to build a broader university-wide eLabFTW user community, and has given presentations on ELN adoption and eLabFTW features to groups at Heidelberg and other universities. The project also participates in national and international ELN networks to exchange experience.
RDM adoption follows from convincing researchers, not mandating them --- groups engage when the benefit to their own work is concrete and immediate. Having a research background can be an asset here, helping to frame RDM in terms of workflows researchers recognise, though good RDM support takes many forms. Investing in networks --- local data stewards, a university-wide ELN community, international practitioner groups --- substantially extends what a small project team can achieve. Finally, this publication itself points to a broader opportunity: INF projects across CRCs have much to learn from one another, and more structured exchange between them would benefit the whole community.
